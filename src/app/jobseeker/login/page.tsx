...
export default function JobSeekerLoginPage() {
  ...
  useEffect(() => {
    if (error) {
      gsap.fromTo(errorRef.current, { opacity: 0, y: -20 }, { opacity: 1, y: 0, duration: 0.3, ease: "power2.out" });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // no-explicit-any fix: use unknown, narrow with instanceof
      console.error("Login error:", err);
      const message = err instanceof Error ? err.message : "Нэвтрэх үед системийн алдаа гарлаа.";
      setError(message);
    }
  };
  ...
  return (
    <div
      ref={pageRef}
      className="min-h-screen bg-white flex items-center mt-8 px-4 sm:px-8 md:px-10 lg:px-32 2xl:mt-10 md:mt-0"
    >
                ...
                <div className="flex flex-col gap-[25px] w-full lg:w-[564px] mb-[20px]">
                  <div className="flex flex-col gap-[5px]">
                    <div className="flex flex-col gap-[4px]">
                      <div className="h-[27px] text-[#0C213A] text-[14px] sm:text-[16px] font-poppins">Имэйл хаяг</div>
                    </div>
                    <input
                      name="email"
                  ...
                  <div className="flex flex-col gap-[5px]">
                    <div className="flex flex-col gap-[4px]">
                      <div className="h-[27px] text-[#0C213A] text-[14px] sm:text-[16px] font-poppins">Нууц үг</div>
                    </div>
