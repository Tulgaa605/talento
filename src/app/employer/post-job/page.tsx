...
const AiIconPlaceholder = () => (
  <ComputerDesktopIcon className="w-6 h-6 text-slate-700" />
);
const UserIconPlaceholder = () => (
  <UserCircleIcon className="w-7 h-7 text-slate-700" />
);
const TalentoLogoNav = () => (
  <ServerStackIcon className="w-7 h-7 text-slate-700" />
);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Ажлын байр нийтлэхэд алдаа гарлаа."
        );
      }
